...
export async function setCredentialPriority(
  ...
  const { data } = await api.post<SuccessResponse>(`/credentials/${id}/priority`, {
    priority,
  } as SetPriorityRequest);
  return data;
}
...
export async function resetCredentialFailure(
  ...
  const { data } = await api.post<SuccessResponse>(`/credentials/${id}/reset`, undefined, {
    signal,
  });
  return data;
}
...
export async function forceRefreshToken(
  ...
  const { data } = await api.post<SuccessResponse>(`/credentials/${id}/refresh`, undefined, {
    signal,
  });
  return data;
}
...
export async function importTokenJson(
  ...
  const { data } = await api.post<ImportTokenJsonResponse>("/credentials/import-token-json", req);
  return data;
}
