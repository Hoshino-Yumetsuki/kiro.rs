...
export function Dashboard({ onLogout }: DashboardProps) {
  ...
  const cachedBalanceMap = new Map(cachedBalancesData?.balances.map((b) => [b.id, b]) ?? []);
  ...
  const allVisibleSelected =
    ...
    sortedCredentials.every((credential) => selectedIds.has(credential.id));
  const someVisibleSelected = sortedCredentials.some((credential) =>
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                onClick={
                  queryingInfo ? () => cancelQueryInfoRef.current?.() : handleQueryCurrentPageInfo
                }
                size="sm"
                variant="outline"
              ...
              >
                <Wallet
                  className={`h-4 w-4 mr-2 ${queryingInfo ? "animate-pulse" : ""}`}
                  aria-hidden="true"
                />
                {queryingInfo
                  ? `取消查询… ${queryInfoProgress.current}/${queryInfoProgress.total}`
                  : "查询信息"}
              </Button>
              <Button
      ...
      {/* 导入凭据对话框 */}
      <ImportTokenJsonDialog open={importDialogOpen} onOpenChange={setImportDialogOpen} />

      {/* 全局配置对话框 */}
      <GlobalConfigDialog open={globalConfigDialogOpen} onOpenChange={setGlobalConfigDialogOpen} />
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {selectedIds.size - pendingBatchDeleteIds.length > 0 &&
                `（将跳过 ${selectedIds.size - pendingBatchDeleteIds.length} 个未禁用凭据）`}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
