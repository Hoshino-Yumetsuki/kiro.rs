...
export function ImportTokenJsonDialog({ open, onOpenChange }: ImportTokenJsonDialogProps) {
  ...
  const normalizeKamAccount = useCallback((item: unknown): unknown => {
    ...
    if (typeof obj.refreshToken === "string" && typeof obj.credentials === "undefined") {
      ...
      return {
        ...
        userId: typeof obj.userId === "string" || obj.userId === null ? obj.userId : undefined,
        nickname,
  ...
  const flattenKamAccount = useCallback(
    (account: Record<string, unknown>): TokenJsonItem | null => {
  ...
  const parseJson = useCallback(
    (text: string): TokenJsonItem[] | null => {
      try {
  ...
  const readJsonFiles = useCallback(
  ...
  const runVerification = useCallback(
    async (results: ImportItemResult[]) => {
      ...
      for (const item of addedItems) {
        ...
        void queue.add(async () => {
          ...
          setVerifyResults((prev) =>
            prev.map((r, idx) => (idx === resultIdx ? { ...r, status: "verifying" } : r)),
          );
          ...
          {step === "input" && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <label htmlFor="importJsonText" className="text-sm font-medium">
                  直接粘贴 JSON
                </label>
                <textarea
                  id="importJsonText"
                  ...
                  placeholder={
                    '粘贴 Kiro Account Manager 导出的 JSON\n\n支持 KAM 1.8.3+ 新版平铺格式：\n[\n  {\n    "email": "...",\n    "refreshToken": "...",\n    "clientId": "...",\n    "clientSecret": "...",\n    "region": "us-east-1"\n  }\n]\n\n也支持旧版嵌套格式：\n{\n  "version": "1.5.0",\n  "accounts": [\n    {\n      "email": "...",\n      "credentials": {\n        "refreshToken": "...",\n        "clientId": "...",\n        "clientSecret": "...",\n        "region": "us-east-1"\n      }\n    }\n  ]\n}'
                  }
                  value={jsonText}
                    ...
                    style={{
                      width:
                        verifyProgress.total > 0
                          ? `${(verifyProgress.current / verifyProgress.total) * 100}%`
                          : "0%",
                    }}
                  />
                </div>
