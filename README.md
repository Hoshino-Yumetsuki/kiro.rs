...
| `enableCredentialCooldown` | boolean | `true` | 是否启用凭据冷却机制。禁用后 429 限流不会触发冷却，仍会尝试故障转移 |
| `enableRateLimit` | boolean | `true` | 是否启用速率限制节流（每日上限 / 请求间隔 / 指数退避）。禁用后凭据级主动节流完全放行，不影响 429 冷却 |
